...
namespace egads.tools.utils
{
	public static class Utilities2D
	{
		...
		{
			if (vertices.Length == 0) { return default(Rect); }
		...
		public static Vector2 GetRandomSpawnPosInFront(Vector2 spawnPos, float radius)
		{
			...
			if (randomOffset.y > 0) { randomOffset.y = -randomOffset.y; }
			if (randomOffset.y > -0.1f) { randomOffset.y = -0.1f; }
		...
		public static float Vector2SqrDistance(Vector2 fromPos, Vector2 toPos) => (fromPos - toPos).sqrMagnitude;
