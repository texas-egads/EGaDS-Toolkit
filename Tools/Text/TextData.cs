...
namespace egads.tools.text
{
	...
	{
        #region Modified Content Properties

        private bool _contentWasModified = false;
		public bool contentWasModified => _contentWasModified;
        ...
        public string Text(string key) => GetTextForLanguage(key, TextManager.currentLanguage);
		...
		public List<KeyValuePair<string, string>> GetMultilanguageText(string key)
		{
			...
			{
				if (language.Value.ContainsKey(key)) { list.Add(new KeyValuePair<string, string>(language.Key, language.Value[key])); }
			}
		...
		{
			if (_infoData.ContainsKey(languageId) && _infoData[languageId].ContainsKey(key)) { return _infoData[languageId][key]; }

			if (_infoData.ContainsKey(TextManager.standardLanguage) && _infoData[TextManager.standardLanguage].ContainsKey(key)) { return _infoData[TextManager.standardLanguage][key]; }
		...
		public bool HasKey(string key)
		{
			...
			if (!_infoData.ContainsKey(TextManager.standardLanguage)) { return false; }
		...
		{
			if (languageId == "") { languageId = TextManager.currentLanguage; }
			if (string.IsNullOrEmpty(languageId) || !TextManager.HasLanguage(languageId)) { return; }
			if (string.IsNullOrEmpty(key)) { return; }
			if (!_infoData.ContainsKey(languageId)) { AddLanguage(languageId); }
			...
			{
				if (!HasLanguage(TextManager.standardLanguage)) { AddLanguage(TextManager.standardLanguage); }
		...
		public void SetMultilanguageText(string key, List<KeyValuePair<string, string>> text)
		{
			...
			{
				if (!_infoData.ContainsKey(item.Key)) { AddLanguage(item.Key); }
		...
		public void DeleteAllExceptThese(List<string> keys)
		{
			for (int i = 0; i < _languages.Count; i++)
			{
				...
				{
					if (!keys.Contains(item.Key)) { deleteList.Add(item.Key); }
				}
		...
		public void BuildIndex()
		{
			...
			for (int i = 0; i < _languages.Count; i++)
			{
				...
				foreach (var item in _textData[i]) { languageEntries.Add(item.Key, item.Value); }
		...
		public void ImportJSON(JSONObject importData)
		{
			...
			if (importData != null)
			{
				foreach (var language in importData)
				{
					...
					foreach (var item in language.Value.Obj) { _infoData[language.Key][item.Key] = item.Value.Str; }
				}
			}
		...
		{ 
			...
			foreach (var language in _infoData)
			{
				...
				foreach (var item in language.Value) { languageData.Add(item.Key, new JSONValue(item.Value.Trim())); }
		...
		private bool HasLanguage(string languageId) => _languages.Contains(languageId);
