...
namespace egads.tools.extensions
{
	...
	public static class RectExtensions
	{
		...
		public static Rect ProjectOntoOther(this Rect rect, Rect parent)
		{
			...
			if (rect.x < parent.x) { result.x = 0; }
			else if (rect.x > parent.xMax) { result.x = 1.0f; }
			else { result.x = (rect.x - parent.x) / parent.width; }

			if (rect.xMax < parent.x) { result.xMax = 0; }
			else if (rect.xMax > parent.xMax) { result.xMax = 1.0f; }
			else { result.xMax = (rect.xMax - parent.x) / parent.width; }

			if (rect.y < parent.y) { result.y = 0; }
			else if (rect.y > parent.yMax) { result.y = 1.0f; }
			else { result.y = (rect.y - parent.y) / parent.height; }


			if (rect.yMax < parent.y) { result.yMax = 0; }
			else if (rect.yMax > parent.yMax) { result.yMax = 1.0f; }
			else { result.yMax = (rect.yMax - parent.y) / parent.height; }
		...
		public static Vector2 Delta(this Rect rect, Rect otherRect) => new Vector2(otherRect.x - rect.x, otherRect.y - rect.y);
