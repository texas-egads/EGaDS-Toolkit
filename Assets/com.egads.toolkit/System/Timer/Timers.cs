...
namespace egads.system.timer
{
    ...
    public class Timers
	{
		...
		{
			if (_timersInRunning.Contains(timer)) { _timersInRunning.Remove(timer); }
			if (_timersInRunningAndSequence.Contains(timer)) { _timersInRunningAndSequence.Remove(timer); }
			if (_timersForAllStates.Contains(timer)) { _timersForAllStates.Remove(timer); }
		}

		public void Update()
		{
			if (MainBase.isRunningOrInSequence) { UpdateTimers(_timersInRunningAndSequence); }
			else if (MainBase.isRunning) { UpdateTimers(_timersInRunning); }
			else { UpdateTimers(_timersForAllStates); }
		}
