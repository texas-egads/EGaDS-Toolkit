...
namespace egads.tools.json
{
    ...
    public class JSONValue
	{
		...
		public static implicit operator JSONValue(double number) => new JSONValue(number);
		...
		public static implicit operator JSONValue(bool boolean) => new JSONValue(boolean);
    ...
    public class JSONArray : IEnumerable<JSONValue>
	{
		...
		{
			if (index >= 0 && index < values.Count) { values.RemoveAt(index); }
			else { JSONLogger.Error("index out of range: " + index + " (Expected 0 <= index < " + values.Count + ")"); }
		}
		...
		public static JSONArray operator +(JSONArray lhs, JSONArray rhs)
		{
			...
			foreach (var value in rhs.values) { result.Add(value); }
			return result;
		}
    ...
    public class JSONObject : IEnumerable<KeyValuePair<string, JSONValue>>
	{
		...
		public JSONObject(JSONObject other)
		{
			...
			if (other != null)
			{
				foreach (var keyValuePair in other.values) { values[keyValuePair.Key] = new JSONValue(keyValuePair.Value); }
			}
		}
			...
			for (var startPosition = 0; startPosition < jsonString.Length; ++startPosition)
			{
				...
				switch (state)
				{
					...
					case JSONParsingState.Value:
						...
						if (c == '"') { state = JSONParsingState.String; }
						else if (char.IsDigit(c) || c == '-') { state = JSONParsingState.Number; }
						else
						{
							switch (c)
							{
                        ...
                        // To re-evaluate this char in the newly selected state
                        --startPosition; 
						break;
					...
					case JSONParsingState.Boolean:
						...
						{
							if (jsonString.Length < startPosition + 4 || jsonString[startPosition + 1] != 'r' || jsonString[startPosition + 2] != 'u' || jsonString[startPosition + 3] != 'e') { return Fail("true", startPosition); }
							switch (currentValue.Type)
							{
						...
						{
							if (jsonString.Length < startPosition + 5 || jsonString[startPosition + 1] != 'a' || jsonString[startPosition + 2] != 'l' || jsonString[startPosition + 3] != 's' || jsonString[startPosition + 4] != 'e') { return Fail("false", startPosition); }
							switch (currentValue.Type)
							{
								case JSONValueType.Object:
					...
					case JSONParsingState.Null:
						...
						{
							if (jsonString.Length < startPosition + 4 || jsonString[startPosition + 1] != 'u' || jsonString[startPosition + 2] != 'l' || jsonString[startPosition + 3] != 'l') { return Fail("null", startPosition); }
							switch (currentValue.Type)
							{
								case JSONValueType.Object:
		...
		private static double ParseNumber(string str, ref int startPosition)
		{
			...
			double result;
			if (!double.TryParse(str.Substring(startPosition, endPosition - startPosition), System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out result)) { return double.NaN; }

            startPosition = endPosition - 1;
			return result;
		}

		private static JSONObject Fail(char expected, int position) => Fail(new string(expected, 1), position);
		...
		public IEnumerator<KeyValuePair<string, JSONValue>> GetEnumerator() => values.GetEnumerator();
